...
class _TaskViewState extends State<TaskView> {
  ...
  dynamic isTaskAlreadyExistUpdateTask() async {
    // if (widget.task != null) {
    //   // Nếu task đã tồn tại, cập nhật nó
    //   try {
    //     title = widget.taskControllerForTitle?.text ?? widget.task?.title;
    //     subtitle =
    //         widget.taskControllerForSubtitle?.text ?? widget.task?.subtitle;

    //     time = time ?? widget.task?.createdAtTime;
    //     date = date ?? widget.task?.createdAtDate;
    //     widget.task?.save();
    //     Firestore_Datasource()
    //         .updateTasks(widget.task!.id, title, subtitle, time, date)
    //         .then((_) {
    ...
    //   }
    // } else {
    //   // Nếu task chưa tồn tại, thêm mới nó
    ...
    //     Firestore_Datasource().addTasks(subtitle, title, time, date).then((_) {
    //       Navigator.of(context).pop();
    //     }).catchError((error) {
    ...
    if (widget.task != null) {
      // Nếu task đã tồn tại, cập nhật nó
      try {
        // Lấy giá trị từ TextField hoặc giữ nguyên giá trị cũ
        title = widget.taskControllerForTitle?.text?.isNotEmpty == true
            ? widget.taskControllerForTitle?.text
            : widget.task?.title;
        subtitle = widget.taskControllerForSubtitle?.text?.isNotEmpty == true
            ? widget.taskControllerForSubtitle?.text
            : widget.task?.subtitle;

        time = time ?? widget.task?.createdAtTime;
        date = date ?? widget.task?.createdAtDate;

        // Gọi Firestore để cập nhật task
        widget.task?.save();
      ...
      if (title != null && subtitle != null) {
        try {
          ...
          // Lưu task vào Hive và Firestore
          BaseWidget.of(context).dataStore.addTask(task: task);
          await Firestore_Datasource().addTasks(subtitle!, title!, time, date);
